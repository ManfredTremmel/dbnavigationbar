 ...
 *        Structure Type
 * @param <F>
 *        AbstractParent panel
 * @author Manfred Tremmel
 * @version 1.0.0, 2011-02-19
 */
public abstract class AbstractDBHeadPosTemplateUI<E extends AbstractDomainHeadPosDB, F extends AbstractParent>
    extends AbstractBasicDBTemplateUI<E, F>
{
  ...
  protected final void setNewPositionButton()
  {
    ...
    if (this.newPositionButton == null)
    {
      this.newPositionButton =
          new Button(this.getConstantsPos().addPositionButton(),
              new ClickHandler()
              {
                @Override
                public void onClick(final ClickEvent event)
                {
                  AbstractDBHeadPosTemplateUI.this.fillPosition(
  ...
  protected final void fillPositions(final E entry)
  {
    ...
    {
      for (int i = this.getPosTable().getRowCount() - 1; i > entry.getKeyPos().length && i > 0; i--)
      {
        this.getPosTable().removeRow(i);
      }
  ...
  protected final PushButton getDeleteButton()
  {
    final PushButton deleteButton = new PushButton(
        new Image(AbstractBasicDBTemplateUI.IMAGES.deletePosition()),
        new ClickHandler()
        {
          @Override
          public void onClick(final ClickEvent event)
          {
            final PushButton sender = (PushButton) event.getSource();
   ...
   *        new constantsPos
   */
  public final void setConstantsPos(final DBHeadPosTemplateUIConstants newConstantsPos)
  {
